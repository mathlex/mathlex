...
\begin{document}
  ...
  \begin{table}[!h]
    ...
    \begin{tabular}{|c|c|c|l|}
      ...
      \hline
      And & \( p \wedge q \) & \texttt{p and q}, \texttt{p \&\& q} & Conjunction \\
      Or & \( p \vee q \) & \texttt{p or q}, \texttt{p || q} & Disjunction \\
      Exclusive Or & \( p \oplus q \) & \texttt{p xor q} & Exclusion \\[4pt]
      Implies & \( p \rightarrow q \) &
        \begin{tabular}{c}
          \texttt{p implies q}, \texttt{p -> q}, \\
          \texttt{if p then q}
        \end{tabular} & Conditional \\[12pt]
      Implied By & \( q \leftarrow p \) &
        \begin{tabular}{c}
          \texttt{q if p}, \texttt{q when p}, \\
          \texttt{q whenever p}, \texttt{q <- p}
        \end{tabular} & Reverse Conditional \\[8pt]
      If And Only If & \( p \leftrightarrow q \) & \texttt{p iff q}, \texttt{p <-> q} & Biconditional \\
      \hline
      ...
      Wedge Product & \( \vec{a} \wedge \vec{b} \) & \texttt{\&v a \&w \&v b} & \\
      Tensor Product & \( T \otimes S \) & \texttt{T \&ox S} & \\
      Direct Sum & \( A \oplus B \) & \texttt{A \&o+ B} & \\
      \hline
      Universal Quantifier & \( \forall x P(x) \) & \texttt{forall x ,~P(x)} & ``For all \ldots'' \\
      Existential Quantifier & \( \exists x P(x) \) & \texttt{exists x :~P(x)} & ``There exists \ldots such that'' \\
      Unique Quantifier & \( \exists ! x P(x) \) & \texttt{unique x :~P(x)} & ``There exists a unique \ldots such that'' \\
      \hline
    \end{tabular}
  \end{table}
  ...
  \begin{table}[!h]
    ...
    \begin{tabular}{|c|c|c|l|}
      ...
      \hline
      Divides & \( p \mid q \) & \texttt{p|q}, \texttt{p divides q} & \\
      Not Divides & \( p \nmid q \) &
        \begin{tabular}{c}
          \texttt{p!|q}, \texttt{p ndivides q}, \texttt{p ndivide q} \\
          \texttt{p notdivides q}, \texttt{p notdivide q}
        \end{tabular} & \\
      Ratio Equality & \( \mathrm{as} \) & \texttt{as} & \\
      \hline
      Subset & \( \subseteq \) & \texttt{subset} & \\
      Superset & \( \supseteq \) & \texttt{superset, supset} & \\
      Proper Subset & \( \subset \) &
        \texttt{propersubset, propsubset, psubset} & \\[6pt]
  ...
  \begin{table}[!h]
    ...
    \begin{tabular}{|c|c|c|l|}
      ...
      \hline
      Combination & \( \binom{n}{r} \) & \texttt{C(n,r)} & Binomial Coefficient, ``choose''\\
      Permutation & \( P(n,r) \) & \texttt{P(n,r)} & \\
      \hline
      ...
      Partial Derivative & \( \displaystyle \frac{\partial}{\partial x} \left( f(x,y) \right) \) & \texttt{pdiff(f(x,y), x)} & \\[8pt]
      Indefinite Integral & \( \displaystyle \int f(x) \,\mathrm{d}x \) &
        \begin{tabular}{c}
          \texttt{int(f(x),x)} \\
          \texttt{\&int f(x) \&dx}
        \end{tabular} & \\[8pt]
      Definite Integral & \( \displaystyle \int_a^b f(x) \,\mathrm{d}x \) &
        \begin{tabular}{c}
          \texttt{int(f(x),x,a,b)} \\
          \texttt{\&int \&\_a \&\^{}b f(x) \&dx}
        \end{tabular} & \\[8pt]
  ...
  \begin{table}[!h]
    ...
    \begin{tabular}{|c|c|c|l|}
      ...
      Set Difference & \( a \setminus b \) & \texttt{a \textbackslash \ b} & \\
      Direct Sum & \( a \oplus b \) & \texttt{a \&o+ b} & \\
      Cartesian Product & \( a \times b \) & \texttt{a \&* b} & \\
      \hline
    \end{tabular}
  \end{table}
  ...
  \begin{table}[!h]
    ...
    \begin{tabular}{|c|c|c|l|}
      ...
      And & \( p \wedge q \) & \texttt{p and q}, \texttt{p \&\& q} & Conjunction \\
      Or & \( p \vee q \) & \texttt{p or q}, \texttt{p || q} & Disjunction \\
      Exclusive Or & \( p \oplus q \) & \texttt{p xor q} & Exclusion \\
      Not & \( \neg p \) & \texttt{not p, \tttilde p, !p} & Logical Negation \\[4pt]
      Implies & \( p \rightarrow q \) &
        \begin{tabular}{c}
          \texttt{p implies q}, \texttt{p -> q}, \\
          \texttt{if p then q}
